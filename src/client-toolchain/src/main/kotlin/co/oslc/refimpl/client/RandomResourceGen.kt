...
/**
 * Generator that can access base URI information for creating cross-references
 */
class SimpleResourceGenWithBaseUri<T : AbstractResource>(
    private val generator: (ServiceProvider, Int, Int, String) -> T,
    private val baseUri: String
) : RandomResourceGen<T> {
    override fun generate(sp: ServiceProvider, n: Int): List<T> = (1..n).map { i -> generator(sp, i, n, baseUri) }
}
...
fun genRequirementWithBaseUri(sp: ServiceProvider, id: Int, max: Int, baseUri: String): Requirement {
    val r = Requirement()
    r.apply {
        shortTitle = "${sp.identifier.uppercase(Locale.getDefault())}-R$id"
        identifier = "req_$id"
        ...
        constrainedBy = randomRequirementLinkWithBaseUri(max, id, baseUri)
        elaboratedBy = randomRequirementLinkWithBaseUri(max, id, baseUri)
    }
    return r
}
...
fun randomRequirementLinkWithBaseUri(maxId: Int, exceptId: Int, baseUri: String): Set<Link> {
    val id = randomIdExcept(maxId, exceptId)
    // Convert catalog URI to service URI
    // e.g., "https://rm.refimpl.oslc.ldsw.eu/services/catalog/singleton" 
    //   ->  "https://rm.refimpl.oslc.ldsw.eu/services/Requirement/sp_single/req_10"
    val serviceBaseUri = baseUri.replace("/catalog/singleton", "")
    val to = "$serviceBaseUri/Requirement/sp_single/req_$id"
    return singleLinkTo(to)
}
